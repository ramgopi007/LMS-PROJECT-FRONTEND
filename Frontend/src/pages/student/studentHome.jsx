...
const StudentHome = ({ student, refreshStudent }) => {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      formData.append("profile", file); // Must match upload.single("profile")

      await axios.post("http://localhost:5000/lms/student/upload-profile-picture", formData, { 
  ...
  const profileImageUrl = student.profilePicture 
    ...
    <div className="max-w-5xl mx-auto p-6">
      ...
      <div className="bg-white rounded-2xl shadow-lg flex flex-col md:flex-row border border-gray-100 overflow-hidden">
        <div className="p-8 bg-gray-50 flex flex-col items-center border-b md:border-b-0 md:border-r border-gray-100">
          ...
          <p className="mt-4 font-bold text-blue-600 uppercase text-sm tracking-widest">{student.role}</p>
        </div>
        <div className="p-8 flex-1">
          <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">About Me</h2>
          <p className="text-gray-700 mb-6 leading-relaxed">{student.bio || "No bio added."}</p>
